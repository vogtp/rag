...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range _q.withNamedCollections {
		if err := _q.loadCollections(ctx, query, nodes,
...
func (_q *UserQuery) WithNamedCollections(name string, opts ...func(*CollectionQuery)) *UserQuery {
	query := (&CollectionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedCollections == nil {
		_q.withNamedCollections = make(map[string]*CollectionQuery)
	}
	_q.withNamedCollections[name] = query
	return _q
}
